<script lang="ts">
  ...
  async function prep_db() {
    ...
    await conn.query(`DROP TABLE IF EXISTS wordcounts;`)
    ...
    await conn.query(`CREATE TABLE wordcounts AS SELECT "@id" id, 
  ...
  async function get_year(year) {
    ...
    const res = await conn.query(`
      SELECT * FROM parquet_scan('new.parquet') WHERE year == ${year}
    `)
...
//        nearest = null;
        ...
        `)*/
        const closest = await conn.query(`SELECT * FROM top_matches NATURAL JOIN overlappings ORDER BY cosine_sim DESC`)
        nearest = []        
...
<div class="mx-20 prose my-20">
  <p>
  This is the first demonstration I've made of possibilities for mixing the <a href="https://nonconsumptive.org">nonconsumptive</a>
  format for distributing collections of text with the new possibilities of in-browser computation offered by 
  <a href="https://duckdb.org/2021/10/29/duckdb-wasm.html">DuckDB's WASM module</a>.
</p>
<p>
  The data here is are State of the Union addresses from the last 90 years; this page loads all of them
  in a small (2.7MB) file including all metadata and pre-computed wordcounts, and then runs some basic 
  queries so that as you read. 
  </p>
...
<div class="flex">
  ...
  <div class="flex flex-row flex-wrap">
    {#each years as y}
    <div class="m-1 p-1 bg-blue-100 hover:bg-blue-400 hover:text-white text-black text-sm m-1 p-1 rounded" 
      on:click={() => { year = y }}>
        {y}
    </div>
    {/each}
